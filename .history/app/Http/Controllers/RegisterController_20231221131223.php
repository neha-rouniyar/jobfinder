...
class RegisterController extends Controller
{
    ...
    public function companyRegisterStore(Request $request)
    {
        ...
        $companyData->name=$rules['cName'];
        $companyData->email=$rules['email'];
        $companyData->password=Hash::make($request->password);
        $companyData->address=$rules['panNo'];
        $companyData->contact=$rules['contact'];
        $companyData->skills=$rules['address'];
        $companyData->experience=$rules['c_desc'];
        ...
        $emailVerification->token =$token;
            ...
            $emailVerification->expiry_at=$tokenExpiry;
            $emailVerification->save();
        return back()->with('registerSuccess','Company Registered Successfully. We have sent you an email');
    }
